...
app = Flask(__name__)
# its ugly and not thread save to use global variables, please use app.config
...
# Load the drinks list (only once) CH why is this not done in create_drinks_table()?
def initialize_drinks(): # arg b/c app.config["drinks_df"] is global
...
def home():
    ...
    categories = app.config["drinks_df"]['strCategory'].unique() ## Extract unique categories from the DataFrame
    return render_template('home.html', categories=categories, drinks=app.config["drinks_df"].to_dict('records'))
...
if __name__ == '__main__':
    app.run(debug=False, port=5001)  # NOTE: runs Flask in debug mode (please let me know if you would like another way for this such as two .py files for Flask and TkInter)
